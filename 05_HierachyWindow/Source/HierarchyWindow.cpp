...
namespace Urho3D
{
	HierarchyWindow::HierarchyWindow(Context* context) : Window(context)
	{
		...
		UIElement* titleBar = CreateChild<UIElement>("HW_TitleBar");
		titleBar->SetInternal(true);
		titleBar->SetFixedHeight(16);
		titleBar->SetLayoutMode(LM_HORIZONTAL);

		titleText_ = titleBar->CreateChild<Text>("HW_TitleText");
		titleText_->SetInternal(true);
		titleText_->SetText("Hierarchy Window");

		closeButton_ = titleBar->CreateChild<Button>("HW_CloseButton");
...
// 		}
	...
	void HierarchyWindow::SetUIElement(UIElement* rootui)
	{
		...
		else
		{
			...
			UnsubscribeFromEvent(rootui, E_ELEMENTADDED);
			UnsubscribeFromEvent(rootui, E_ELEMENTREMOVED);
